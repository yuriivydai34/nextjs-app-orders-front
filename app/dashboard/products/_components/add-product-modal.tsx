    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">New product</h3>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
          <Field label="Name *" name="header" required />
          <Field label="Article" name="article" />
          <Field label="Picture URL" name="picture" placeholder="https://..." />

          <div className="grid grid-cols-2 gap-3">
            <Field label="Price (₴) *" name="price" type="number" required />
            <Field label="Discount price (₴)" name="price_discount" type="number" defaultValue="0" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Volume *" name="measurement" type="number" required />
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Unit *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Product type *</label>
              <select name="type_product" required className={selectCls}>
                {TYPE_PRODUCTS.map((v) => <option key={v} value={v}>{v}</option>)}
              </select>
            </div>
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Packaging *</label>
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Juice type</label>
