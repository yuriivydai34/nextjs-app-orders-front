  ...
  return (
    <>
      ...
      <div className="flex items-center gap-1.5">
        <ActionBtn onClick={() => viewRef.current?.showModal()}>View</ActionBtn>
        <ActionBtn onClick={() => { setError(null); editRef.current?.showModal(); }}>Edit</ActionBtn>
        <ActionBtn
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Product #{product.id}</h3>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            <ViewField label="Name" value={product.header} wide />
            <ViewField label="Article" value={product.article} />
            <ViewField label="Price" value={`${product.price} ₴`} />
            <ViewField label="Discount price" value={product.is_discount ? `${product.price_discount} ₴` : '—'} />
            <ViewField label="Volume" value={`${product.measurement} ${product.type_measurement?.toLowerCase() ?? ''}`} />
            <ViewField label="Type" value={product.type_product} />
            <ViewField label="Packaging" value={product.type_packaging} />
            <ViewField label="Juice type" value={product.type_juice} />
            {product.shipment_weight != null && (
              <ViewField label="Shipment (kg)" value={String(product.shipment_weight)} />
            )}
            {product.description && (
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Edit Product #{product.id}</h3>
        ...
        <form onSubmit={handleEditSubmit} className="px-5 py-4 space-y-3">
          <Field label="Name" name="header" defaultValue={product.header} />
          <Field label="Article" name="article" defaultValue={product.article ?? ''} />
          <div className="grid grid-cols-2 gap-3">
            <Field label="Price (₴)" name="price" type="number" defaultValue={String(product.price)} />
            <Field label="Discount price (₴)" name="price_discount" type="number" defaultValue={String(product.price_discount)} />
          </div>
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Discount active</label>
            <select
              name="is_discount"
          ...
          <Field label={`Volume (${product.type_measurement?.toLowerCase() ?? ''})`} name="measurement" type="number" defaultValue={String(product.measurement)} />

          {error && <p className="text-xs text-red-600 dark:text-red-400">{error}</p>}

          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={() => editRef.current?.close()} className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">Cancel</button>
      ...
      >
        <div className="px-5 py-6 space-y-4">
          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">Delete product?</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            <span className="font-medium text-gray-700 dark:text-gray-300">{product.header}</span> will be permanently removed.
          </p>
          {error && <p className="text-xs text-red-600 dark:text-red-400">{error}</p>}
          <div className="flex justify-end gap-2">
            <button onClick={() => deleteRef.current?.close()} className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">Cancel</button>
